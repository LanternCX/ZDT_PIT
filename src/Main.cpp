...
void loop() {
  ...
  // Emm_V5_Read_Sys_Params(0, S_VER);
  Serial.println("Waiting ...");
  // 等待返回命令，命令数据缓存在数组rxCmd上，长度为rxCount
  Emm_V5_Receive_Data(rxCmd, &rxCount);
  Serial.println("Got res. cnt: " + String((int) rxCount)) ;
  for (int i = 0; i < rxCount; i++) {
    Serial.println(String(i) + ":" + String(rxCmd[i]));
  }
